...
<br />
<br />
The respondents completed an online questionnaire to assess each of 52 graphs depicting 52 species' annual occupancy across Great Britain from 1900 to 2016, having first read the Introduction text (**<span style="color: blue;">refer to where this can be found</span>**). Respondents were asked to separately score their level of agreement (on a five point scale) of two periods for each of the 52 species' plots: 1900-1976 and 1976-2016 (i.e., before and after UKBMS' standardized monitoring was initiated). We used a five point scale over a continuous score so that the results would be comparable across respondents. Respondents were instructed to indicate their agreement or disagreement that “the pre-1976 trend (including the uncertainty ribbon) is a plausible description of how this species changed since 1900”, and that "the post-1976 trend reflects existing knowledge for this species".
<br />
<br />
Disagreement with an occupancy trend indicates that it is not a plausible description nor/or adequately reflect the evidence. 
<br />
The **ten** respondents scored both periods for each of 52 species' occupancy plots (i.e., 104 questions per respondent) for a total of 1,040 responses. These were coded from -2 (strongly disagree) to +2 (strongly agree).
<br />
...
Next,we calculated the mean and variance for each respondent across questions (i.e. pre-1976 and post-1976) and species, as well as separately for each question (i.e. pre-1976 and post-1976) across the 52 species (Figure 1).
<br/>
<br/>
<br/>
...
ggplot(df2, aes(Mean_all, Variance_all), by = spp_name) +
  ...
  labs(x = "Mean",
       y = "Variance",
       color = "Rerspondent ID", 
       title ="Figure 1: Mean scores against variance per respondent",
...
* On average across species, all respondents score post-1976 trends (diamond) higher than pre-1976 trends (triangle). 
* All but one respondents' (ID 09) mean score across the two time periods (circles) are greater than zero meaning that these respondents average scores were generally better than ‘Neither agree nor disagree’.<br/>
* All respondents, on average, scored post-1976 trends above zero, indicating that generally, there is some level of agreement/plausibility for the late time period. <br/>
* Three of ten respondents' mean scores for the 'pre-1976' periods (triangle) are negative (04, 09 and 10), i.e., no better than ‘Neither agree nor disagree’ and instead, more likely to be disagreed, than agreed, with.  <br/>
    + One respondent (ID 09) stands out as scoring lower than others across all questions.<br/>
<br/>   
...
* Most respondents' difference between pre-1976 trends' (triangle) and post-1976 trends' (diamond) score variances are around, or just above, 0.05; however, three respondents relative variance differences deviate from the generality. Namely, ID01, ID06 and ID10, whose answers tend to be more consistent/unanimous to questions on the later time series while answers for the early part of the time series vary more depending on species.<br/>
<br />
<br />
<br />

***
Next, we also calculated the mean and variance for each species across respondents, and measured the unanimity across respondents as 1-variance (Figure 2).
of the time series vary more depending on species.<br/>
<br />
<br />


```{r figure2-3, eval=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
# Load required package(s)
require(tidyr)
require(dplyr)
require(ggplot2)
...
ggplot() + # Empty ggplot function to allow use of different data sets for plotting 
  ...
  labs(x = "Mean Score", y = "Unanimity",
       title ="Figure 2: Mean scores against unanimity (1- variance)") +
...
ggplot(df3b, aes(value, group = spp_name)) + geom_histogram(bins = 15, aes(fill = spp_name))  + 
  ...
  labs(x = "Score", y = "Count",
       title ="Figure 3: Mean scores and unanimity scores by time period",
...
* 16 of the 104 questions have a mean score of zero or lower meaning that the respondents average score was no better than ‘Neither agree nor disagree’. 
...
* Five species' time periods on the top left-hand side of Figure 2 have a mean score of -1 or lower and high unanimity scores indicating low divergence of opinion among respondents. These are all WSCs, specifically: <br />
...
* As for the criterion/cut off point beyond which we won't include model outputs: Instead of discarding, let's say, species' model outputs with mean scores of -1 or less, perhaps a 'stronger' way of presenting it would be to say e.g.:
    ...
    + Second, we only discard species/model outputs when there is high unanimity among respondents (because this is strong evidence for something being wrong; however, if unanimity is low although the mean score falls below 0, most respondents may have answered 'Disagree', whilst the one-two expert(s) on that specific species answered 'Strongly agree'). 
<br/>
<br/>
<br/>
...
<br/>
<br/>
```{r figure4a, eval=TRUE, echo=FALSE, cache=TRUE}
# Load required package(s)
require(dplyr)
require(ggplot2)
...
ggplot(df4, aes(value, group = spp_name)) + geom_histogram(bins = 15, aes(fill = spp_name))  + 
  facet_wrap(~ Difference) +
  labs(x = "Score", y = "Count",
       title ="Figure 4: Distance between pre-1976 vs. post-1976 scores",
...
```{r figure4b, eval=TRUE, echo=TRUE, cache=TRUE}
head(df4mean[order(df4mean$value, decreasing = TRUE), ])
tail(df4mean[order(df4mean$value, decreasing = TRUE), ])
head(df4unanimity[order(df4unanimity$value, decreasing = TRUE), ])
tail(df4unanimity[order(df4unanimity$value, decreasing = TRUE), ])
```
...
```{r figure5, eval=FALSE, echo=FALSE, cache=TRUE, message=FALSE}
# Load required package(s)
require(png)
...
*** 
Let's take a closer look at the species with low unanimity indicating high divergence of opinion among respondents to potentially find whether it's driven by one respondent's answer (and could be a potential outlier/mistake to follow up on) or 'genuine' disagreement among the participants. 
<br/>

We use a box plot to visualise the distribution of scores for species and time period combinations where the unanimity score is less than zero, only including combinations with outliers to more easily see the most extreme cases (Figure 5). Combinations without outliers are discarded as 'genuine' disagreement among the participants.   
<br/>
<br/>
<br/>
```{r figure6, eval=TRUE, echo=FALSE, cache=TRUE}
# Load required package(s)
require(tidyr)
require(dplyr)
require(ggplot2)
...
df6 %>% 
  ...
  geom_boxplot(outlier.colour = "red") +
  labs(x = "Species & time period", y = "Agreement Scores",
       title ="Figure 6: Distribution of scores",
...
* **<span style="color: red;">Galina: Add book title and section</span>**
...
* Note the -3 residual for participant 05 (suspected mistake; Figure 5). 
* Neither participant 04 nor 10 (also suspected mistakes; Figure 5) look like they have 'extreme' residuals. <br/>
