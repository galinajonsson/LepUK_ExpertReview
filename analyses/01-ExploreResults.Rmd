...
### Graph
q <- ggplot(df3, aes(Mean, Unanimity)) + # Empty ggplot function to allow use of different data sets for plotting 
...
<br/>
<br/>
```{r msfigure4, eval=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
library(ggpubr)
ggarrange(p, q, 
          labels = c("(A)", "(B)"),
          ncol = 1, nrow = 2)
#ggsave("../plots/msFigure4.png",width=7,height=8)
```
<br/>
<br/>
...
# add colour grouing
df_test2$scoregroup <- "pos"
df_test2$scoregroup[df_test2$spp_name=="Aglais.urticae"] <- "badbad"
df_test2$scoregroup[df_test2$spp_name=="Pieris.napi"] <- "badbad"
df_test2$scoregroup[df_test2$spp_name=="Pieris.rapae"] <- "badbad"
df_test2$scoregroup[df_test2$spp_name=="Polyommatus.icarus"] <- "badbad"
df_test2$scoregroup[df_test2$spp_name=="Maniola.jurtina"] <- "badbad"

df_test2$scoregroup[df_test2$spp_name=="Lycaena.phlaeas"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Polyommatus.coridon"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Plebejus.argus"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Euphydryas.aurinia"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Thymelicus.sylvestris"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Aglais.io"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Coenonympha.pamphilus"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Hipparchia.semele"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Ochlodes.sylvanus"] <- "bad"
df_test2$scoregroup[df_test2$spp_name=="Satyrium.w-album"] <- "bad"
...
ggplot(df_test2, aes(value, group = spp_name, fill= scoregroup)) + geom_histogram(bins = 14)  + 
  facet_grid(period ~ Difference ) +
  #facet_wrap(~ Difference) +
  labs(x = "Score", y = "Species") +
   #    title ="Figure 4: Mean scores and unanimity scores by time period") +
...
#### Visualise all scores as box plots

```{r allscores-boxplot, message=FALSE, eval=TRUE, echo=TRUE, cache=TRUE}
require("tidyverse")

dfAllAscores <- df[,c("spp_name", "name", "value")]
names(dfAllAscores)[names(dfAllAscores) == "name"] <- "Period"
dfAllAscores[dfAllAscores$Period=="q1", "Period"] <- "1900-1976"
dfAllAscores[dfAllAscores$Period=="q2", "Period"] <- "1976-2016"

dfAllAscores$spp_name <- gsub(".", " ", dfAllAscores$spp_name, fixed=TRUE)

# Box plot
ggplot(dfAllAscores, aes(x=fct_rev(spp_name), y=value)) + 
  geom_boxplot() + facet_grid(. ~ Period) + coord_flip() + 
  labs(x = "Species", y = "Score") + theme_light() +
  theme(strip.text.x = element_text(color="black", face="bold")) +
  theme(axis.text.y = element_text(face = "italic"))

#ggsave("../plots/AllScoreBoxPlot.png",width=7,height=9)
```
